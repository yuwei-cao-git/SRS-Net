...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Perform super-resolution of Sentinel-2 with DSen2",
    ...
    parser.add_argument(
        ...
        action="store",
        dest="input_file",
        help="An input Sentinel-2 data file",
    )
    parser.add_argument(
        "--output",
        action="store",
        dest="output_file",
        help="output super resolution 10m data file",
    )
    ...
    outdata = driver.Create(
        args.output_file, ds10.RasterXSize, ds10.RasterXSize, 12, gdal.GDT_UInt16
    )
    ...
    for i in tqdm(range_i):
        data10 = ds10.ReadAsArray(
            xoff=0, yoff=int(i * 6), xsize=ds10.RasterXSize, ysize=6 * chunk_size
        )[validated_10m_indices, :, :]

        data20 = ds20.ReadAsArray(
            xoff=0, yoff=int(i * 3), xsize=ds20.RasterXSize, ysize=3 * chunk_size
        )[validated_20m_indices, :, :]

        data60 = ds60.ReadAsArray(
            xoff=0, yoff=int(i), xsize=ds60.RasterXSize, ysize=chunk_size
        )[validated_60m_indices, :, :]
        ...
        data10 = data10 / 2000.0
        data20_ = data20_ / 2000.0
        data60_ = data60_ / 2000.0

        test_2X = model_2X.predict([data10[np.newaxis, ...], data20_[np.newaxis, ...]])
        test_6X = model_6X.predict(
            [
                data10[np.newaxis, ...],
                data20_[np.newaxis, ...],
                data60_[np.newaxis, ...],
